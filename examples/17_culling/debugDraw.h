...
static void debugDrawBVTreeNode(
  ...
  const T3DBvhNode *node, const T3DFrustum *frustum, int level
) {
  if((node->value & 0b1111) == 0) {
    if(t3d_frustum_vs_aabb_s16(frustum, node->aabbMin, node->aabbMax)) {
      int dataOffset = node->value >> 4;
      debugDrawAABB(fb, node->aabbMin, node->aabbMax, vp, DEBUG_COLORS[level & 7]);
      debugDrawBVTreeNode(fb, vp, &node[dataOffset], frustum, level+1);
      debugDrawBVTreeNode(fb, vp, &node[dataOffset+1], frustum, level+1);
    }
  }
}

static void debugDrawBVTree(uint16_t *fb, const T3DBvh *bvh, T3DViewport *vp, const T3DFrustum *frustum) {
  debugDrawBVTreeNode(fb, vp, bvh->nodes, frustum, 0);
}